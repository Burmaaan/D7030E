package OldCode;// OldCode.Card.java
...
public class Card implements Comparable<Card> {
    ...
    // Helper: choose region by name or index from OldCode.Card.regions
    ...
    public boolean applyEffect(Player active, Player other, int row, int col) {
        ...
        if (placement != null && placement.equalsIgnoreCase("Settlement/city")) {
            ...
            if (type.contains("Unit")) {
                ...
                // Large Trade Ship: adjacency 2-for-1 between L/R regions (handled in OldCode.Server)
                if (nmEquals(nm, "Large Trade Ship")) {
                    active.placeCard(row, col, this);
                    active.flags.add("LTS@" + row + "," + col);
                    return true;
                }
                // “Common” trade ships: 2:1 bank for specific resource (handled in OldCode.Server)
        ...
        // We keep these very small; most are handled in OldCode.Server (events/Brigand etc.)
        if ("Action".equalsIgnoreCase(placement) || "Action".equalsIgnoreCase(type)) {
            ...
            if (nmEquals(nm, "Scout")) {
                // Only meaningful when used with a new settlement (OldCode.Server stores
            ...
            if (nmEquals(nm, "Brigitta the Wise Woman")) {
                // Choose production die result before rolling; we store forced value in OldCode.Server
