package OldCode;// OldCode.Server.java
...
public class Server {
    ...
    public static void main(String[] args) {
        ...
        try {
            if ((args.length == 0 || (args.length > 0 && args[0].equalsIgnoreCase("bot")))) {
                ...
                System.out.println("Usage: java OldCode.Server [optional: bot|online]");
                return;
            }
        } catch (Exception e) {
    ...
    public void start(boolean withBot) throws Exception {
        ...
        // 3) networked players
            ...
            // Use your existing OldCode.OnlinePlayer class for remote players:
            OnlinePlayer op = new OnlinePlayer();
            // Then wire up its socket streams directly:
            op.setConnection(sock, in, out);
            players.add(op);
            System.out.println("Connected Online OldCode.Player ");
            op.sendMessage("WELCOME Online OldCode.Player ");
        }
        initPrincipality();
    ...
    private boolean checkWinEndOfTurn(Player active, Player other) {
        ...
        if (score >= 7) {
            broadcast("winner: OldCode.Player " + players.indexOf(active)
    ...
    private void resolveEvent(int face, Player active, Player other) {
        switch (face) {
            ...
            case EV_TRADE:
                ...
                // OldCode.Player with Trade Advantage (>=3 commerce) gains 1 resource of choice from
                // bank
                for (Player p : players) {
            ...
            case EV_EVENT_A:
            case EV_EVENT_B: {
                broadcast("[Event] Draw Event OldCode.Card");
